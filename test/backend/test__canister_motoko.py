...
def test__session_password_create(network: str) -> None:
    response = smoketest(
        canister_name=CANISTER_NAME,
...
def test__session_password_create_anonymous(
    network: str, identity_anonymous: None  # pylint: disable=unused-argument
) -> None:
    """Verify we receive #err (401) (Unauthorized) for anymous principal"""
    _response = smoketest(
        canister_name=CANISTER_NAME,
        canister_method="session_password_create",
        network=network,
